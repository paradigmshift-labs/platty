...
- `using-platty` 코어 룰 준수: npm으로 전역 설치된 `platty` 사용, `--json`, `nextAction.command` 추종, Stop Conditions.
- 전역 CLI가 `UNKNOWN_COMMAND`/`UNEXPECTED_ERROR`를 반환하면 다른 실행 경로로 우회하지 않고 전역 CLI 재설치/재빌드 필요성을 보고한다.
