...
코드와 문서화 대상이 많으면 기술 문서, EPIC, 비즈니스 문서 생성에 시간이 오래
걸리고 AI 프로바이더의 사용량 한도에 도달할 수 있습니다. 특히 Claude Code 같은
구독형 프로바이더는 요금제에 따른 사용량 제한으로 생성이 일시 중단될 수
있습니다.

중단되더라도 처음부터 다시 생성할 필요는 없습니다. Platty는 완료된 작업과 진행
상태를 저장하므로, 프로바이더 사용량이 다시 확보된 뒤 워크플로를 실행하면 완료된
작업은 유지하고 미완료 작업부터 이어서 생성합니다. 실패한 작업이 있으면 Platty가
안내하는 복구 명령을 실행한 뒤 계속할 수 있습니다.
