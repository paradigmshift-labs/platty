...
import { ensureCanonicalStaticSnapshot } from '@/pipeline_modules/sync/static_map.js'
...
export class BuildDocsGenerationRuntime {
  ...
  async start(input: BuildDocsStartInput): Promise<{
    ...
    // Bootstrap a baseline static snapshot from the canonical DB if the project has none yet, so the
    // documents generated by this (first) run get stamped with a documentSourceHash. Keeps the first
    // run in `full` mode (one snapshot < the incremental threshold of 2) and avoids the legacy
    // run → sync static-map → run → docs dance.
    ensureCanonicalStaticSnapshot(db, input.projectId)
