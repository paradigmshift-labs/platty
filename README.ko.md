...
## 첫 분석(SOT 생성)은 오래 걸릴 수 있습니다

온보딩 때 플래티는 코드베이스 전체를 분석해 서비스 백과사전(SOT)을 만듭니다. 이
최초 1회는 원래 오래 걸리고 토큰도 많이 쓸 수밖에 없는 작업입니다. 그럼에도
분석해 둘 값어치가 있습니다.

- **한 번은 전체를 봐야 합니다** — 수십만 줄의 코드와 여러 레포를 연결해 코드
  그래프를 그리고, 이를 자연어 백과사전으로 요약합니다. 새로 온 CTO·CPO가 회사의
  코드 전체를 파악하는 그 일을, 사람 대신 처음 딱 한 번 해두는 것입니다.
- **한 번 해두면 계속 씁니다** — 이후 모든 기획·개발 에이전트는 코드베이스를
  매번 다시 읽지 않고 이 백과사전만 봅니다. 그래서 작업할 때마다 오류는 절반
  이하, 토큰은 10분의 1 이하로 줄어듭니다. 처음 든 비용은 이후 모든 작업에서
  회수됩니다.
- **두 번째부터는 가볍습니다** — 코드가 바뀌어도 전체를 다시 분석하지 않고 변경된
  부분을 중심으로 백과사전을 갱신합니다. 무거운 건 최초 1회뿐, 그 다음부터는
  빠르고 저렴합니다.

최초 온보딩 때 소모되는 토큰은 "한 번 내고 계속 아끼는 투자"입니다. 중간에 사용량
한도에 걸려 멈춰도 완료된 문서는 유지되므로, 다시 실행하면 미완료 작업부터 이어서
생성합니다.

회사나 기관에서 사용하는 대규모 프로젝트라면 플래티 팀으로 문의해 주세요. 별도
API를 제공해 무료로 분석하실 수 있도록 도와드리겠습니다.
