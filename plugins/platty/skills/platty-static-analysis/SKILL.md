...
Keep calling `platty status --project <project> --json` between phases. When status reports `build_docs`, switch to `platty-docs-target-curation` or `platty-generated-docs`.
...
- `build_docs`: route to `platty-docs-target-curation` or `platty-generated-docs`
