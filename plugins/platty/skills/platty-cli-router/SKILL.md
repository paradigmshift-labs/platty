...
1. If CLI output includes `nextCommand` or `nextAction.command` at the top
   ...
   pause. Gate precedence overrides returned commands for EPIC approval,
   incomplete target review, failed `build_docs` recovery, active
   generated-output work before sync, or recovery that must preserve an
   existing run.
...
3. Static analysis no longer has a public `confirm` step. Compatibility note:
   ...
   reinstall or update of the global @pshift/platty package.
...
- A command from the table fails with `UNKNOWN_COMMAND` or `UNEXPECTED_ERROR`: stop and report that the installed global CLI may be stale or the command may not exist. Reinstall or update the global @pshift/platty package before continuing; do not substitute a guessed command or execution path.
