...
- `system_design.md`가 `sdd-design.v2`이고 design-mode readiness validator가 95점 이상,
  critical 0이다. 이 검증에는 `tasks.md`가 필요하지 않다.
- Figma 근거가 있는 설계라면 상단 레지스트리가 모든 exact Figma node를 보존하고, 모든
  Figma-sensitive UI/interaction task가 해당 surface ID와
  `Figma node -> R/AC -> US/scenario -> design decision -> task` 연결을 보존한다.

하나라도 실패하면 신규 `tasks.md`를 만들지 않는다. 기존 artifact는 validator의
`--mark-stale` 경로로 `status: stale`를 원자적으로 기록하고
...
Figma 연결 설계에서만 이 절을 둔다. 승인된 `system_design.md`의 전체 Figma 표면 레지스트리를
그대로 한 번 투영하며, 이 문서에서 사용하는 node ID는 모두 여기에 있어야 한다. 개별 태스크는
`FIGMA-SURFACE-*`만 참조하고 URL·node 목록을 반복하지 않는다.

| surface ID | canonicalUrl | fileKey | exact Figma node IDs | expected sourceRevision | required live reads | receipt |
| --- | --- | --- | --- | --- | --- | --- |
| FIGMA-SURFACE-01 | `<node-specific Figma URL>` | `<fileKey>` | `<nodeId>, <nodeId>` | `<sourceRevision>` | `screenshot + bounded design context` | `<queriedAt / observedSourceRevision / stable>` |

- [ ] §1의 일반 실행 전 확인과 함께 `platty-mcp-figma-design-sync` 계약으로 각 exact node의 현재 screenshot과 bounded design context를 다시 조회한다.
- [ ] observed sourceRevision이 expected sourceRevision과 같고 node identity 및 필수 live read가 모두 완전한지 확인한다.
- [ ] 인증·권한·read 실패, node 누락, identity mismatch, screenshot/design context 누락 또는 drift면 코드를 수정하지 않는다.
- [ ] 실패한 surface를 `stale/blocked`로 보고하고 Figma refresh → alignment → 새 system design revision → design approval로 되돌린다.

이 표가 단일 실행 preflight와 receipt 위치다. 실패하면 개별 태스크를 고치지 않고 영향을 받는
surface와 구현 섹션을 보고한다.
...
Figma 연결 설계에서는 문서 상단 `Figma 구현 기준`의 모든 referenced surface가 `stable`인지
확인한다. 하나라도 실패하면 코드를 고치지 않고 stale 대상과 영향을 받는 섹션을 보고한다.
...
- [ ] RED — expected failure: `<exact command>` — <기능 부재로 실패하는 assertion>
- [ ] Minimal implementation: `<exact file path>`의 `<exact symbol>`에 RED를 통과시키는 최소 변경만 한다.
- [ ] GREEN: `<same exact command>` — <통과할 핵심 assertion>
- [ ] Regression: `<exact adjacent command>` — <보존할 기존 동작>
- [ ] Self-review — spec coverage: 관련 R/AC/CHG/VER와 오류·권한·데이터 계약이 빠짐없이 연결됐는지 확인한다.
- [ ] Commit checkpoint: 위 RED/GREEN/Regression과 self-review가 모두 통과한 하나의 응집된 변경 경계를 기록한다.
...
- [ ] Figma: `FIGMA-SURFACE-01` — exact nodes는 상단 레지스트리 참조 — R-01 / AC-01 → US-01 / SC-01 → DEC-01 → TASK-UI-01
- [ ] Figma MCP preflight before code edits: `FIGMA-SURFACE-01`의 current design context와 screenshot을 조회하고 상단 receipt를 기록한다.
- [ ] preflight read 실패·node 누락·identity mismatch·drift면 이 화면 구현을 중단하고 `stale/blocked`로 보고한다.
...
위 Figma 행은 승인된 설계에 Figma 연결이 있고 해당 화면 또는 상호작용이 그 근거에
민감할 때만 쓴다. Figma-sensitive UI/interaction task는 exact Figma node를 사용한다.
관련 없는 backend, data, 운영 작업에는 node를 반복하지 않는다. Figma 입력이 없는 설계는
기존 task 형식을 그대로 사용한다.
...
13. 각 변경은 `exact file path → RED(expected failure) → minimal implementation → GREEN →
    regression → self-review(spec coverage) → commit checkpoint` 순서의 작은 실행 단위다.
    커밋 체크포인트는 실행 권한을 대신하지 않으며, 실제 실행 세션의 승인 범위 안에서만 수행한다.
14. Figma 근거가 있는 UI·interaction 작업은 상단 레지스트리의 `FIGMA-SURFACE-*`를 참조해
    exact node와 `Figma node -> R/AC -> US/scenario -> design decision -> task`를 보존하고,
    코드 수정 전 live screenshot/design context 조회와 expected sourceRevision drift gate를 실행한다.
...
10. Figma 연결이 있는 설계라면 상단 레지스트리가 모든 exact node, Figma MCP preflight,
    stable receipt를 보존하고, 모든 Figma-sensitive UI/interaction task가 surface ID와 전체
    제품·설계 추적 체인을 보존한다.
