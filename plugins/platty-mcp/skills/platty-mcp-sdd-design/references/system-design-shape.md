...
> **Lifecycle: `<DRAFT — 개발자 검토 필요 | APPROVED — exact revision 승인 완료>`.** 입력: `prd.md`, `user_stories.md`.
> 구현 준비도: `<ready | partial | blocked>` — <한 문장 이유>.
> 상세 코드·SSOT·영향도 근거: 부록 A 및 `prd.md` §9.

승인 기록 시 lifecycle 문구도 현재 상태로 교체한다. 승인된 문서에 `DRAFT`, `승인 대기`,
또는 현재 `tasks.md`가 없다는 문구를 남기지 않는다.
...
부여하며, 결정되지 않은 의제에 `DEC-*`를 미리 부여하지 않는다. 다만 되돌릴 수 있고 승인된
사용자 결과를 보존하는 구현 선택은 제품 승인자에게 묻는 회의 의제가 아니다. 원문 근거를
확인해 `DEC-*`로 닫는다. 해당하는 `O-*` 또는 예외적 `TQ-*`가 없으면 예시 행을 삭제하고
`미결 의제 없음`이라고 쓴다.

### 기획에서 넘겨받은 설계 결정 패킷

| 인계 ID | 바뀌면 안 되는 사용자 결과 | 확인한 AS-IS·근거 | 검토한 구현안 | 위험 등급 | 처리 | 연결 ID |
| --- | --- | --- | --- | --- | --- | --- |
| DH-01 | | | | reversible | DEC-01 | D-01 / R-01 / CHG-01 |

각 `DH-*`는 `DEC-*`, Evidence-Resolution, 또는 아래 허용 사유가 적힌 `TQ-*` 하나로
처분한다. API·DTO·DB·index·query·정렬 구현·tie-breaker·cache·component·file·test·배포·
rollback 선택만 남았고 사용자 결과가 같다면 `TQ-*`로 남기지 않는다.

### 기술 설계 시작 결정표

이 표는 깊은 코드 조사 전에 만든 킥오프 패킷과 이후 확인 결과를 연결한다. 대화 로그를
복사하지 않고, 기획에서 넘겨받은 질문을 누가 어떻게 닫았는지만 보여 준다. 최종 설계 승인은
이 표의 답변과 별개이며, 완성된 `system_design.md`의 현재 revision을 읽은 뒤에만 받는다.

| 인계·기술 ID | 시작 시 분류 | 시작 시 추천·답변 | 근거 또는 다음 확인 | 설계 반영 결과 |
| --- | --- | --- | --- | --- |
| DH-01 | auto-decision | <사용자 결과를 보존하는 추천안> | <기존 근거 또는 bounded read> | DEC-01 |
| DH-02 | evidence-resolution | <agent가 확인할 사실> | <정확한 다음 read> | ER-01 또는 DEC-02 |
| TQ-01 | technical-owner-question | <추천안과 trade-off> | <허용 사유·결정자> | <답변 또는 open> |

모든 `DH-*`는 `DEC-*`, Evidence-Resolution, `TQ-*` 중 정확히 하나로 이어져야 한다.
`TQ-*`는 설계 시작 시 한 번의 통합 결정표로 답을 받고, 중간 조사에서 새 예외 질문이 생기면
현재 revision을 중단하고 새 킥오프로 돌아간다. 질문이 없으면 예시 `TQ-*` 행을 삭제한다.
...
`FigmaDesignAlignmentPacket`이 있을 때만 이 절을 작성한다. 없으면 절 전체를 생략한다.
Figma MCP의 시각·구조 근거는 제품 승인을 대신하지 않으며, 확인된 의미 연결만 설계에
사용한다.

아래 표는 이 설계가 사용하는 전체 Figma 표면 레지스트리다. 한 표면에 여러 상태·컴포넌트
node가 필요하면 `exact Figma node IDs`에 모두 적는다. 일부 node를 개별 태스크에만 숨기지
않는다.

| surface ID | canonicalUrl | fileKey | exact Figma node IDs | reportId | expected sourceRevision |
| --- | --- | --- | --- | --- | --- |
| FIGMA-SURFACE-01 | `<node-specific Figma URL>` | `<fileKey>` | `<nodeId>, <nodeId>` | `<reportId>` | `<sourceRevision>` |

| surface ID / Figma node | 관찰 근거 | R/AC | US/scenario | design decision | 정합성 상태 |
| --- | --- | --- | --- | --- | --- |
| FIGMA-SURFACE-01 / `<exact nodeId>` | <화면·컴포넌트·상태·동작> | R-01 / AC-01 | US-01 / SC-01 | DEC-01 | aligned |

연결 규칙은 `Figma node -> R/AC -> US/scenario -> design decision`이다. 필요한 행이
누락되거나 제품 문서와 충돌하면 `aligned`로 쓰지 않고 §11 Evidence-Resolution 또는
제품 수정 경로로 보낸다. 배치·색상·라벨만으로 정책, 권한, 예외 규칙을 발명하지 않는다.

#### 구현 시점 Figma MCP Preflight

Figma-sensitive 화면·상호작용마다 구현자가 코드 수정 전에 실행할 live-read 계약을 작성한다.
이 표가 없으면 해당 화면은 구현 준비가 완료되지 않은 것이다.

| surface ID | 적용 화면·상호작용 | required live reads | 실패·drift 처리 |
| --- | --- | --- | --- |
| FIGMA-SURFACE-01 | SCREEN-01 | `exact Figma node IDs의 screenshot + bounded design context` | 코드 수정 중단 → Figma evidence refresh → alignment → 새 design revision/approval |

구현 시점에는 `platty-mcp-figma-design-sync` 계약으로 현재 screenshot과 bounded design
context를 다시 읽고 observed sourceRevision을 계산한다. 인증·권한·read 실패, node 누락,
identity mismatch, 필수 live read 누락, source drift가 있으면 기존 capture나 task 문구로
구현하지 않는다. 영향받는 설계를 `stale/blocked`로 보고하고 evidence refresh와 alignment를
다시 거친 뒤 새 revision을 승인받는다. Figma 입력이 없는 설계에서는 이 절을 생략한다.
...
| DEC ID | 기술 결정·수용 위험 | 인계·제품·요구 근거 ID | 이유·source evidence | 영향 표면·CHG | 결정 주체 | 범위·재검토 조건 |
| --- | --- | --- | --- | --- | --- | --- |
| DEC-01 | | DH-01 / D-01 / R-01 / AC-01 | | API-01 / CHG-01 | 설계 | |

### 기술 미결 질문과 다음 확인

| TQ ID | 기술 질문·위험 | 영향 ID | 사람 결정이 필요한 허용 사유 | 결정할 기술 소유자 | 결정 전 확인 |
| --- | --- | --- | --- | --- | --- |
| TQ-01 | | | <비용·운영 책임 / 보안·개인정보 / 데이터 손실 / 비가역 migration> | | 부록 A-3~A-7 |

`TQ-*`는 source read만으로 닫을 수 없고 비용·운영 책임, 보안·개인정보, 데이터 손실,
비가역 migration 중 하나가 실제로 달라질 때만 쓴다. 제품 결과·범위·규칙·AC·성공 판정이
달라지면 `TQ-*`가 아니라 제품 문서 개정으로 돌린다. 단순 구현 선호나 선택지 존재는 사람
결정 사유가 아니다.
...
- 모든 PRD `DH-*`는 `DEC-*`, Evidence-Resolution, 또는 허용 사유와 기술 소유자가 명시된
  `TQ-*`로 정확히 한 번 처분한다. 승인된 사용자 결과를 보존하는 되돌릴 수 있는 구현안은
  bounded source read 뒤 설계가 결정하며 제품 승인자에게 선택지로 되묻지 않는다.
- `ready` 전에는 `scripts/readiness-validator.mjs --mode design --design <system_design.md>
  --json`으로 현재 설계만 검증한다. 승인 전에는 임시 task projection을 생성하거나 기존
  `tasks.md`를 읽지 않는다. 승인 후 실제 `tasks.md`는
  `scripts/readiness-validator.mjs --mode tasks --tasks <tasks.md> --json`으로 별도 검증한다.
  tasks 모드는 같은 디렉터리의 `system_design.md`를 자동으로 찾는다. 95점 미만 또는 critical finding
