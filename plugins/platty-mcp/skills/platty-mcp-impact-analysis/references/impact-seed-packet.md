...
Search candidates separately and retain `matchedQuery` for every candidate hit.
Zero results only means the pattern lacks an indexed anchor; it does not prove
the behavior, file, or impact is absent.
...
Exact behavior claims require the bounded exact source read. If workspace or
source tools are missing, record the MCP capability gap and return `partial`;
use no local fallback.
