...
- **Expected GREEN route**: `platty-mcp-sdd-spec` invokes impact analysis, writes `prd.md §9`, and adds a compact request link.
- **Owner reference**: `impact-dossier.md` persisted impact artifact.
- **Observable pass criteria**: `prd.md §9` retains the full evidence packet while `prd.md` contains only the status link and a user-relevant limit.
