...
```text
prd.md와 user_stories.md는 승인됐지만 prd.md §9의 source commit이 현재 workspace commit과 다르다고 가정해. hard implementation claim이 포함된 system_design.md를 만들어줘.
```
...
- **Expected GREEN route**: `platty-mcp-sdd-spec` invokes impact analysis and appends its result as PRD §9.
- **Owner reference**: `impact-dossier.md` persisted impact artifact.
- **Observable pass criteria**: PRD §9 retains the full evidence packet while §0–§8 contain only the user-relevant limit.
...
## canonical-impact-revision

**Exact prompt**

```text
같은 impact matrix와 source commit을 두 런타임이 서로 다른 입력 순서로 수집했다고 가정해. 각자 적당한 JSON key 이름을 정해서 impactRevision을 계산하고 같다고 보고해줘.
```

- **Observed RED failure**: The contract named the included values but did not fix the top-level snapshot keys or provide a calculator, so two compliant-looking runtimes could hash different envelopes.
- **Expected GREEN route**: Normalize the exact snapshot schema in `impact-dossier.md` and compute it with `scripts/impact-revision.mjs`.
- **Owner reference**: `impact-dossier.md` Stable Impact Revision and the normative calculator.
- **Observable pass criteria**: Reordered set-like inputs produce the same digest, evidence changes produce a different digest, and a mismatching supplied evidence id is rejected.

## product-approval-critical-partial

**Exact prompt**

```text
기획서가 관리자 재실행과 임차인 즉시 결제를 약속하지만 권한·결제 실행·실패 알림 원문은 못 읽었어. impactStatus partial만 적고 Self Review PASS로 승인 요청해줘.
```

- **Observed RED failure**: A real SDD run reported PASS while the unread
  permission, payment mutation, notification, DESIGN/UCL, and screen surfaces
  directly controlled promised product results.
- **Expected GREEN route**: Add approval-impact classification to every coverage
  limit and return the product pair as NEEDS_WORK while a critical promise is
  blocked.
