#!/usr/bin/env python3
"""Validate and render grounded Interview 3 screen and element behavior records."""
import argparse
import copy
...
from legacy import interview1
from legacy import interview2
from legacy.interview1 import load_json
from legacy.interview2 import digest

ROOT = Path(__file__).resolve().parents[2]
TEMPLATE = ROOT / 'schemas/interview-3.template.json'
...
def check_shape(value, shape, path, errors):
    if isinstance(shape, OneOf):
        ...
        interview1.check_shape(value, shape, path, errors)


NULL_REF = OneOf(str, type(None))
ASSIGNMENT = {'scope_id':str, 'item_ref':NULL_REF, 'axis_id':str, 'value_id':str}
SOURCE = copy.deepcopy(interview2.SOURCE)
SOURCE['provider'] += ('interview_2',)
...
PACKET = copy.deepcopy(interview2.SHAPE['decision_packets'][0])
PACKET['decision_scope'] = {'target_refs':[str]}
SHAPE = {
    'schema_version':(1,), 'model_profile':('screen_element_behavior_v1',),
    'case_id':str,'title':str,'status':interview2.SHAPE['status'],
    'input_binding':{'interview_1':BINDING,'interview_2':BINDING},
    'evidence_status':copy.deepcopy(interview2.SHAPE['evidence_status']),
    'sources':[SOURCE],
    ...
    'issues':[{'id':str,'question':str,'target':('interview_1','interview_2','interview_3','wireframe','development'),
        'blocking':bool,'reason':str,'areas':[str], 'target_refs':[str],
        'action':copy.deepcopy(interview2.SHAPE['issues'][0]['action'])}],
    'reviews':[REVIEW],'review':{key:REVIEW for key in CRITERIA},
    'confirmation':copy.deepcopy(interview2.SHAPE['confirmation']),
    'history':copy.deepcopy(interview2.SHAPE['history']),
}
...
def _inputs(input_path):
    ...
    path1 = path2.with_name('01-context.json')
    one, two = load_json(path1), load_json(path2)
    if not interview1.validate(one)['complete'] or not interview2.validate(two)['complete']:
        raise ValueError('input interviews 1 and 2 must be complete and currently confirmed')
    h1, r1 = interview1.fingerprints(one)
    binding = two['input_binding']
    if (Path(binding['interview_1_path']).resolve() != path1
            or binding['content_hash'] != h1 or binding['review_hash'] != r1
        ...
        raise ValueError('input interview 2 is not mutually bound to current adjacent interview 1')
    return one, two, path1, path2


def bind_inputs(data, input_path):
    one, two, path1, path2 = _inputs(input_path)
    for stage, source, path, module in (('interview_1',one,path1,interview1),('interview_2',two,path2,interview2)):
...
def validate(data):
    ...
    try:
        ...
        one, two, path1, path2 = _inputs(binding['interview_2']['path'])
        for stage, source, path, module in (('interview_1',one,path1,interview1),('interview_2',two,path2,interview2)):
    ...
    for source in sources.values():
        ...
        require((source['provider'] in ('interview_1','interview_2')) == (source['kind']=='imported'), path, 'imported kind and provider must agree')
    ...
    for issue in data['issues']:
        ...
        if issue['blocking'] or issue['target'] in ('interview_1','interview_2','interview_3'):
            gaps.append(path+': unresolved issue')
    if data['status']=='waiting':
        require(any(i['blocking'] and i['action']['kind'] in ('wait','handoff')
            and i['target'] in ('interview_3','wireframe','development') for i in data['issues']),
        ...
        require(not any(i['action']['kind'] in ('ask_user','platty','web')
            or i['target'] in ('interview_1','interview_2') for i in data['issues']),
    ...
    report.update(valid=not errors,ready_for_confirmation=ready,complete=complete,completion_errors=completion,
        ...
        blocking_issue_ids=[i['id'] for i in data['issues'] if i['blocking'] or i['target'] in ('interview_1','interview_2','interview_3')],
...
def init_from_interview2(output, input_path=None):
...
def main():
    ...
    try:
        if args.command=='init':
            init_from_interview2(args.path,args.input); result={'created':True,'path':str(args.path.resolve())}; code=0
