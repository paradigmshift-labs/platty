...
def graph_gaps(data) -> list[str]:
    ...
    for state_id, state in states.items():
        ...
        # `undecided`는 「나가는 길을 상위가 아직 정하지 않았다」이다. 이 값이 없던 동안
        # 한 케이스가 통과하려고 `final`로 바꿔야 했는데, 그것은 「여기서 끝난다」는 다른 주장이다.
        if state.get('kind') not in ('final', 'undecided') and not outgoing[state_id]:
            gaps.append(f'$.experience_states[{state_id}]: nonterminal state needs an outgoing '
                        'transition, or kind: undecided if the upstream has not said where it goes')
